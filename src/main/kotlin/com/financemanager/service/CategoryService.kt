...
class CategoryService(
    ...
    fun createCategory(user: User, request: CreateCategoryRequest): CategoryResponse {
        ...
        if (categoryRepository.existsByNameIgnoreCaseForUser(trimmedName, user)) {
            throw DuplicateResourceException("A category named '$trimmedName' already exists")
        }
    ...
    fun deleteCategory(user: User, name: String): Unit {
        ...
        if (defaultCat.isPresent) {
            throw BadRequestException("Default category '$trimmedName' cannot be deleted")
        }

        val category = categoryRepository.findByNameAndUser(trimmedName, user)
            .orElseThrow { ResourceNotFoundException("Category '$trimmedName' not found") }

        if (transactionRepository.existsByCategoryAndDeletedFalse(category)) {
            throw ConflictException("Category '$trimmedName' cannot be deleted because it has associated transactions")
        }
