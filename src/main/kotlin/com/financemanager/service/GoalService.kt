...
class GoalService(
    ...
    @Transactional
    fun createGoal(user: User, request: CreateGoalRequest): GoalResponse {
        validateTargetDate(request.targetDate)

        val startDate = request.startDate ?: LocalDate.now()
        if (request.startDate != null && request.startDate.isAfter(request.targetDate)) {
    ...
    fun getGoals(user: User): List<GoalResponse> =
        goalRepository.findByUserOrderByTargetDateAsc(user).map { buildGoalResponse(it, user) }

    @Transactional(readOnly = true)
    fun getGoal(user: User, id: Long): GoalResponse {
        val goal = goalRepository.findByIdAndUser(id, user)
            .orElseThrow { ResourceNotFoundException("Goal with id $id not found") }
        return buildGoalResponse(goal, user)
    }

    @Transactional
    fun updateGoal(user: User, id: Long, request: UpdateGoalRequest): GoalResponse {
        val goal = goalRepository.findByIdAndUser(id, user)
            .orElseThrow { ResourceNotFoundException("Goal with id $id not found") }
    ...
    private fun buildGoalResponse(goal: Goal, user: User): GoalResponse {
        val transactions = transactionRepository.findByUserSinceDate(user, goal.startDate)
        ...
        val currentProgress = (totalIncome - totalExpenses).coerceAtLeast(BigDecimal.ZERO)

        val progressPercentage = if (goal.targetAmount > BigDecimal.ZERO) {
            (currentProgress.divide(goal.targetAmount, 4, RoundingMode.HALF_UP) * BigDecimal("100"))
        ...
        val remainingAmount = (goal.targetAmount - currentProgress).coerceAtLeast(BigDecimal.ZERO)
